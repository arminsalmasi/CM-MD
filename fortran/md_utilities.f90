...
!##########################################################
!##########################################################
SUBROUTINE do_Dealloc(xyz, vel, frc, acc, EAM, samp, nSamp)
!##########################################################
! Deallocation of arrays and data types:
! xyz, velocity, force, acceleration,
! EAM Potentials array
! and sampel holder
! to prevent memory leaks / DoS
!##########################################################
  IMPLICIT NONE
  ...
  ! Deallocate timestep variables
  IF(ALLOCATED(xyz)) DEALLOCATE(xyz)
  IF(ALLOCATED(vel)) DEALLOCATE(vel)
  IF(ALLOCATED(acc)) DEALLOCATE(acc)
  IF(ALLOCATED(frc)) DEALLOCATE(frc)
  IF(ALLOCATED(EAM)) DEALLOCATE(EAM)

  IF(ALLOCATED(samp)) THEN
    DO ix = 0, nSamp
      IF(ALLOCATED(samp(ix)%xyz)) DEALLOCATE(samp(ix)%xyz)
      IF(ALLOCATED(samp(ix)%vel)) DEALLOCATE(samp(ix)%vel)
      IF(ALLOCATED(samp(ix)%acc)) DEALLOCATE(samp(ix)%acc)
      IF(ALLOCATED(samp(ix)%frc)) DEALLOCATE(samp(ix)%frc)
      IF(ALLOCATED(samp(ix)%EAM)) DEALLOCATE(samp(ix)%EAM)
    END DO
    DEALLOCATE(samp)
  END IF
...
!##########################################################
  ...
  IF(ALLOCATED(mdiv)) DEALLOCATE(mdiv)
...
!##########################################################
  ...
  IF(ALLOCATED(xyzd)) DEALLOCATE(xyzd)
