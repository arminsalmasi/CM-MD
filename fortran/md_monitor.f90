...
!###############################################################################################
  ...
  ! Input Validation to prevent DoS, crash, or memory corruption
  IF (Temp < 0.0_dp) THEN
    WRITE(*,*) "Error: Temperature must be non-negative."
    STOP 1
  END IF
  IF (nCo < 0 .OR. nNi < 0 .OR. (nCo + nNi) <= 0) THEN
    WRITE(*,*) "Error: Invalid number of atoms. Must be non-negative and total > 0."
    STOP 1
  END IF
  IF (V <= 0.0_dp) THEN
    WRITE(*,*) "Error: Volume must be positive."
    STOP 1
  END IF
  IF (t <= 0.0_dp) THEN
    WRITE(*,*) "Error: Simulation time must be positive."
    STOP 1
  END IF
  IF (dt <= 0.0_dp) THEN
    WRITE(*,*) "Error: Timestep length must be positive (prevents division by zero)."
    STOP 1
  END IF
  IF (sampInt < 0.0_dp) THEN
    WRITE(*,*) "Error: Sample interval must be non-negative."
    STOP 1
  END IF
