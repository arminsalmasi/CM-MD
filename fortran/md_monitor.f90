...
!###############################################################################################
  ...
  ! Security: Input validation to prevent DoS (div-by-zero) and memory corruption
  IF (Temp <= 0.0_dp .OR. nCo < 0 .OR. nNi < 0 .OR. &
      V <= 0.0_dp .OR. t <= 0.0_dp .OR. dt <= 0.0_dp .OR. &
      sampInt <= 0.0_dp .OR. (nCo + nNi) <= 0) THEN
    WRITE(*,*) 'Error: Invalid physical simulation parameters provided.'
    STOP 1
  END IF
