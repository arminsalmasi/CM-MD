...
!##########################################################
  ...
  ! Avoid re-seeding on every call because SYSTEM_CLOCK is too slow
  ! and results in identical seeds for rapid calls.
  ! We just call RANDOM_NUMBER.
...
!##########################################################
  ...
  REAL(dp) :: array(n)
  REAL(dp) :: u(n+1)
  REAL(dp) :: rand_normal2(n)
  REAL(dp) :: pi

  pi = 4.0*ATAN(1.0)
  CALL do_genRand(u,n+1,1)

  DO i = 1, n, 2
    array(i) = SQRT(-2.0*LOG(u(i))) * COS(2*pi*u(i+1))
    IF (i+1 <= n) THEN
      array(i+1) = SQRT(-2.0*LOG(u(i))) * SIN(2*pi*u(i+1))
    END IF
  END DO
