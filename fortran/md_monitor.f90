...
!###############################################################################################
  ...
  ! SECURITY: Validate input parameters to prevent memory corruption and DoS
  ! e.g., negative array allocations or divide-by-zero
  IF (Temp < 0.0_dp) STOP "Security Error: Invalid temperature"
  IF (nCo < 0) STOP "Security Error: Invalid number of Co atoms"
  IF (nNi < 0) STOP "Security Error: Invalid number of Ni atoms"
  IF (V <= 0.0_dp) STOP "Security Error: Invalid cell volume"
  IF (t <= 0.0_dp) STOP "Security Error: Invalid simulation time"
  IF (dt <= 0.0_dp) STOP "Security Error: Invalid timestep"
  IF (sampInt <= 0.0_dp) STOP "Security Error: Invalid sampling interval"
  IF (nCo == 0 .AND. nNi == 0) STOP "Security Error: System must have atoms"
